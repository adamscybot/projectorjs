(function (root, factory) {
    ...
    pInstance.prototype.addOverlay = function (overlay, options) {
        ...
        // Cover decides which element the positining is applied.
        // Allows the user to avoid covering whole video with a div
        // if they want to.
            ...
            ['start', 'end'].forEach(function(boundary) {
                if (typeof timing[boundary] === 'string') {
                    that.element.addEventListener(timing[boundary], function() {
                        ...
                        boundary === 'start' ? overlay.runBeginOverlay(timing, curTime, false) : overlay.runEndOverlay(timing, curTime, false);
                    });
                }
            });
        });
