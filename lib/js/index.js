(function (root, factory) {
    ...
    var pInstance = function (element, config, vjs) {
        ...
        element.addEventListener("fullscreenchange", function() {
            that.overlays.forEach(function(overlay) {
                overlay.setFontSize((that.wrapper.offsetWidth * 0.01)+'px');
            });
        });
    ...
    pInstance.prototype.addOverlay = function (overlay) {
        ...
        ['top', 'right', 'left', 'bottom'].forEach(function(direction) {
            if(overlay.options[direction]) {
                div.style[direction] = overlay.options[direction];
            }
        });
        div.appendChild(overlay.el);
        overlay.wrapper = div;
        overlay.setFontSize((this.wrapper.offsetWidth * 0.01)+'px');
    ...
    var Overlay = (function(){
        ...
        exports.prototype.render = function () {
            ...
            div.setAttribute('class', that.class);
        ...
        exports.prototype.update = function (curTime, dirty) {
            ...
            this.options.timings.forEach(function (timing) {
                ...
                if (!timing.active && curTime >= timing.min && curTime <= timing.max) {
                    runFunc(timing.beforeBeginOverlay, that.el, curTime, dirty);
                    that.beginOverlay();
                    runFunc(timing.afterBeginOverlay, that.el, curTime, dirty);
                    timing.active = true;

                } else if (timing.active && (curTime < timing.min || curTime > timing.max)){
                    runFunc(timing.beforeEndOverlay, that.el, curTime, dirty);
                    that.endOverlay();
                    runFunc(timing.afterEndOverlay, that.el, curTime, dirty);
                    timing.active = false;
                }
            });
        };

        exports.prototype.setFontSize = function (size) {
            this.wrapper.style.fontSize = size;
        };
